import React from 'react';
import { NavLink } from 'react-router-dom';
import useAuthStore from '/src/store/authStore.js';

const AdminSidebar = () => {
  const { user } = useAuthStore();
  const activeLinkClass = 'bg-primary text-white';
  const defaultLinkClass = 'text-text-primary hover:bg-gray-100';

  // Tentukan path utama berdasarkan role
  const basePath = user?.role === 'admin' ? '/admin' : '/instructor';

  return (
    <aside className="w-1/4 lg:w-1/5 flex-shrink-0">
      <div className="bg-white p-4 rounded-lg shadow-md sticky top-24">
        <div className="text-center mb-4 pb-4 border-b border-border">
          <h3 className="font-bold text-lg">{user?.name}</h3>
          <p className="text-sm text-text-muted capitalize">{user?.role}</p>
        </div>
        <nav className="space-y-2">
          {/* Tampilkan Dashboard HANYA untuk Admin */}
          {user?.role === 'admin' && (
            <NavLink
              to="/admin"
              end
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-2 rounded-md font-semibold ${
                  isActive ? activeLinkClass : defaultLinkClass
                }`
              }
            >
              <svg
                className="h-5 w-5"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                  d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"
                />
              </svg>
              Dashboard
            </NavLink>
          )}

          {/* Tampilkan Manajemen Pengguna HANYA untuk Admin */}
          {user?.role === 'admin' && (
            <NavLink
              to="/admin/users"
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-2 rounded-md font-semibold ${
                  isActive ? activeLinkClass : defaultLinkClass
                }`
              }
            >
              <svg
                className="h-5 w-5"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                  d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"
                />
              </svg>
              Pengguna
            </NavLink>
          )}

          {/* Menu Kursus untuk Admin dan Instruktur */}
          <NavLink
            to={`${basePath}/courses`}
            className={({ isActive }) =>
              `flex items-center gap-3 px-4 py-2 rounded-md font-semibold ${
                isActive ? activeLinkClass : defaultLinkClass
              }`
            }
          >
            <svg
              className="h-5 w-5"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth="2"
                d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"
              />
            </svg>
            Kursus
          </NavLink>

          {/* Menu Pendaftaran HANYA untuk Admin */}
          {user?.role === 'admin' && (
            <NavLink
              to="/admin/enrollments"
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-2 rounded-md font-semibold ${
                  isActive ? activeLinkClass : defaultLinkClass
                }`
              }
            >
              <svg
                className="h-5 w-5"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                  d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"
                />
              </svg>
              Pendaftaran
            </NavLink>
          )}
        </nav>
      </div>
    </aside>
  );
};

export default AdminSidebar;
