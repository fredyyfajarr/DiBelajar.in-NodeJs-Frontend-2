    ...
    if (isOpen) {
      if (mode === 'edit' && currentMaterial) {
        reset({
          ...
          testContent: currentMaterial.testContent || [],
        });
      } else {
        reset({
          title: '',
    ...
    <div className="pl-4 space-y-2">
      ...
      {fields.map((item, k) => (
        <div key={item.id} className="flex items-center gap-2">
          <input
            type="radio"
            // --- PERUBAHAN DI SINI: MENGHAPUS '{...register(...)}' ---
            // Kita tidak perlu mendaftarkan radio button ini, cukup state 'checked' dan 'onChange'
            name={`correctOption_${nestIndex}`} // Beri nama unik untuk grup radio button
            onChange={() => {
